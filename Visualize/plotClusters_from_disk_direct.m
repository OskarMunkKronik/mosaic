function [X, Y, Z_refolded, mass_spec, mz_vals_cluster,p] = plotClusters_from_disk( ...
        n_cluster_vec, feature_table, clusters_all_samples_before_duplicate_removal,mzroi_aug, ...
...
for n = 1:n_clusters
    ...
    cluster_str = sprintf(['%0', num2str(n_digits_clusters), 'd'], n_cluster_vec(n));
    ...
    mass_spec{n} = zeros(length(mz_vals_cluster{n}), size(Z_refolded{n},4));
    if faster

        Zk = squeeze(max(Z_refolded{n},[],3:4));
        
        p = plot3(X{n}, Y{n}, Zk, 'Color', col(1,:),'LineWidth',1.5);

        % p = surf(X{n}, Y{n}, Zk','EdgeColor','interp','FaceColor','interp');
        % mass spectrum
        mass_spec{n} = zeros(size(Z_refolded{n},[3,4]));
        mass_spec{n}(:,:) = squeeze(sum(Z_refolded{n}, [1 2]));
       n
view([-10 -30 20])
    else
    for k_count = 1:length(sample_vec)%1:size(Z_refolded{n},4)
        k = sample_vec(k_count);
        % 3D plot of max-projected spatial component
        Zk = squeeze(max(Z_refolded{n}(:,:,:,k),[],3));
