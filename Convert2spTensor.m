% Should go to excell / options file 
% cd C:\Users\mht541\Documents\Python_Scripts\2DLC_workflow\matlab
% addpath C:\Users\mht541\Documents\2D_LC_IMS_R\2DIMS\Matlab\OrganizeData
...
tic
Z_all = Z;
if ~exist(fullfile(Options.Paths.save2mat,'SparseTensor'),'dir')
    mkdir(fullfile(Options.Paths.save2mat,'SparseTensor'))
end 
for k = sample_vec
      fprintf(1,'save sample: %i/%i\n',k,length(sample_vec))
    Z = Z_all{k};
    save(fullfile(Options.Paths.save2mat,'SparseTensor',[fileList(k).name(1:end-10),'_dim_all_samples.mat']), ...
